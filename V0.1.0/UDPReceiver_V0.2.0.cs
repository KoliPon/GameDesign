...
public class UDPReceiver : MonoBehaviour
{
    ...
    [SerializeField] private float aspectRatioThreshold = 0.60f; // 長寬比閾值
    ...
    private string ClassifyGestureWithImprovedLogic(List<Vector2> points, out float score)
    {
        ...
        Debug.Log($"\n[手勢分析] 圓度: {circleRoundness:F3}, 尖銳度: {triangleSharpness:F3}, 角度性: {squareAngularity:F3}, 長寬比: {aspectRatio:F3}");

        // ⭐ 改進 V0.1.0+：優先級判定 + 排他性檢查

        // 1. 圓形判定（優先：最圓最獨特）
        if (circleRoundness > circleThreshold && 
            circleRoundness > triangleSharpness + 0.15f && 
            circleRoundness > squareAngularity)
        {
        ...
        if (triangleSharpness > triangleThreshold && 
            triangleSharpness > circleRoundness && 
        ...
        // 3. 正方形判定（最後判定，最低優先級）
        if (squareAngularity > squareThreshold && 
            aspectRatio > aspectRatioThreshold && 
            squareAngularity > circleRoundness && 
            squareAngularity > triangleSharpness)
        {
